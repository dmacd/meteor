...
Meteor._LivedataConnection = function (url, options) {
  ...
  // Tracks methods which the user has tried to call but whose result messages
  // have not arrived yet. An array of "blocks"; each block has a bool "wait"
  // (telling whether or not this block contained a single "wait" method) and a
  // list "methods" of MethodInvoker objects. The first block in the list
  // contains the methods that are currently in flight and have not received
  // their "result" yet; subsequent blocks contain methods that have not yet
  // been sent. There should not be two adjacent non-wait blocks. Only the first
  // block may contain no methods (which means that it has sent methods, and
  // they all have returned, but not all of them have made their writes visible
  // and called their user callbacks yet).
...
var MethodInvoker = function (options) {
  ...
  self._justResultCallback = options.justResultCallback || function () {};
...
_.extend(MethodInvoker.prototype, {
  ...
  // If the user provided a justResultCallback, call it immediately.
  ...
  receiveResult: function (err, result) {
    ...
    self._justResultCallback(err, result);
...
_.extend(Meteor._LivedataConnection.prototype, {
  ...
  //   justResultCallback: Function - a callback to call as soon as the method
  ...
  apply: function (name, args, options, callback) {
    ...
    var methodInvoker = new MethodInvoker({
      ...
      callback: callback,
      connection: self,
      justResultCallback: options.justResultCallback,
      message: {
        msg: 'method',
        method: name,
        params: args,
    ...
    if (self._resetStores) {
      _.each(self._methodInvokers, function (invoker) {
        if (invoker.gotResult()) {
          ...
          //
          // It's important that these get called BEFORE any callbacks
          // corresponding to methods that are actually sent on this new
          // connection. (This is accomplished by executing
          // _afterUpdateCallbacks in order.) For example, if this method was a
          // login method which received its result (and set onReconnect in its
          // justResult callback), we need to make sure to call its callback
          // *before* the callback associated with the "login with token" method
          // executed by that onReconnect.
